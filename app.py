...
def init_db():
    ...
    if cursor.fetchone()[0] == 0:
        products = [
            ...
            ('Платье летнее', 'Zara', 'M', 'Красный', 3990, 10, 'платья', 'вискоза', 'лето'),
            ('Куртка кожаная', 'Bershka', 'L', 'Черный', 12990, 5, 'куртки', 'кожа', 'осень'),
            ('Свитер теплый', 'Tommy Hilfiger', 'XL', 'Серый', 5990, 12, 'свитеры', 'шерсть', 'зима'),
            ('Шорты спортивные', 'Adidas', 'L', 'Черный', 2990, 20, 'шорты', 'полиэстер', 'лето'),
...
def login():
    if request.method == 'POST':
        ...
        if user:
            ...
            flash('Добро пожаловать в магазин одежды!')
